...
__declspec(naked) int printf(const char *format, ...) {
	...
	__asm {
		...
		mov     edi, dword ptr [esp+4] // Return address
		...
		push    edi
		lea     edi, dword ptr [ret_addr]
		add     edi, SN_DATA_SEC_BLOCK_ADDR
		mov     edi, dword ptr [edi]
		mov     dword ptr [esp+4], edi // Store the return address on the stack again
