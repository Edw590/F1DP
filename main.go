...
func additionalPatches(file_bytes []byte, file_md5 string) bool {
	...
	initial_offset = 0x12ED44
	new_bytes = []byte{0x00, 0x00}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12ED48
	new_bytes = []byte{0x60, 0x14}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12ED4C
	new_bytes = []byte{0x88, 0x13}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_children.Leg_Right.Effect3
	initial_offset = 0x12ED60
	new_bytes = []byte{0x00, 0x00}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12ED64
	new_bytes = []byte{0x60, 0x14}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12ED68
	new_bytes = []byte{0x88, 0x13}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_children.Leg_Left.Effect2
	initial_offset = 0x12EDEC
	new_bytes = []byte{0x00, 0x00}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12EDF0
	new_bytes = []byte{0x60, 0x14}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12EDF4
	new_bytes = []byte{0x88, 0x13}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_children.Leg_Left.Effect3
	initial_offset = 0x12EE08
	new_bytes = []byte{0x00, 0x00}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12EE0C
	new_bytes = []byte{0x60, 0x14}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x12EE10
	new_bytes = []byte{0x88, 0x13}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_super_mutant.Leg_Left.Effect2.FailMessage
	initial_offset = 0x12F3DC
	new_bytes = []byte{0xBA}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_ghoul.Head.Effect5.StatCheck
	initial_offset = 0x12F6C0
	new_bytes = []byte{0xFF, 0xFF, 0xFF, 0xFF}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_brahmin.Head.Effect5.StatCheck
	initial_offset = 0x12FCA8
	new_bytes = []byte{0xFF, 0xFF, 0xFF, 0xFF}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_radscorpion.Leg_Right.Effect2.FailureEffect
	initial_offset = 0x1304E4
	new_bytes = []byte{0x02}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_radscorpion.Leg_Left.Effect2.FailureEffect
	initial_offset = 0x13058C
	new_bytes = []byte{0x02}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_radscorpion.Leg_Left.Effect3.FailMessage
	initial_offset = 0x1305B0
	new_bytes = []byte{0xE8, 0x15}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_centaur.Torso.Effect4.FailureEffect
	initial_offset = 0x13162C
	new_bytes = []byte{0x02}
	patchBytes(file_bytes, initial_offset, new_bytes)

	// Crit_deathclaw.Leg_Left
	initial_offset = 0x132EE4
	new_bytes = []byte{0x04}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x132F00
	new_bytes = []byte{0x04}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x132F1C
	new_bytes = []byte{0x04}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x132F38
	new_bytes = []byte{0x04}
	patchBytes(file_bytes, initial_offset, new_bytes)
	initial_offset = 0x132F54
	new_bytes = []byte{0x04}
	patchBytes(file_bytes, initial_offset, new_bytes)
